jQuery(document).ready(function($) { 
    ...
    if (isValid) {
        ...
        // for screenshot skip
            ...
            // Use event delegation for the nextToStep2 button
            $(document).on('click', '#nextToStep21', function() {
...
// if no screenshot proof use this function
function nextToStepsuccess() { 

        // Prepare FormData for upload
        const formData = new FormData();
        formData.append('action', 'digages_upload_screenshot_skip');
        formData.append('order_id', $('.orderNumberDisplay').first().text().trim());
        formData.append('payment_method_title', $('.nav-linkt.active').first().text().trim());
        formData.append('status', 'on-hold');
        formData.append('nonce', ajax_object.nonce); 
...
// function to send email action details when screenshot is skipped

function nextToStep3skipped() {
    let redirectUrl = window.redirectUrl; // Retrieve the redirect URL set during upload
    ...
    if ($('.nav-linkt.active').attr('id').startsWith('tab-bank')) {
        ...
        const emailFormData = {
            action: 'digages_send_p2p_confirmation_skip',
            order_id: orderId,
            user_email: userEmail,
            bankName: bankName,
            phoneNumber: accountNumber,
