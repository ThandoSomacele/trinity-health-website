...
function digages_direct_pop_payments_enqueue_scripts(){
    ...
    wp_enqueue_style('digages-direct-payments-css', plugin_dir_url(__FILE__) . '../assets/css/digages-direct-payments.css', array(), '2.1.3.1');
    wp_add_inline_style( 'digages-direct-payments-css', ":root { --accent-color: $accent_color; }" );
     
    // Enqueue custom JS to handle modal behavior
    wp_enqueue_script('custom-digages-mobilemoney-modal', plugin_dir_url(__FILE__) . '../assets/js/custom-digages-mobilemoney-modal.js', array('jquery'), '2.1.3', true);
    ...
    wp_enqueue_script('payment-methods', plugin_dir_url(__FILE__) . '../assets/js/payment-methods.js', array('jquery'), '2.1.3', true);
