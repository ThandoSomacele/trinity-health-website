...
<!-- Display bank transfer records -->
    ...
    <!-- Start by adding a bank account below to accept payments via bank transfer. -->
        ...
        if($banklimits > 0)
        {
            echo '<button type="button" class="digages-onboard-addacnt-add-btn digages-onboard-addacnt-popup-trigger"  data-target="paywall">+ Add account</button>';
        }
        else
        {
            echo '<button type="button" class="digages-onboard-addacnt-add-btn digages-onboard-addacnt-popup-trigger" data-target="banktransfer">+ Add account</button>';
        }
?>            
