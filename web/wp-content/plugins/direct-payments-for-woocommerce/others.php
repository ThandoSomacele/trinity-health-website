...
//Do action when screenshot is skipped


// Hook to handle the AJAX requests for both logged-in and non-logged-in users
add_action('wp_ajax_digages_upload_screenshot_skip', 'digages_upload_screenshot_and_update_order_skip');
add_action('wp_ajax_nopriv_digages_upload_screenshot_skip', 'digages_upload_screenshot_and_update_order_skip');

function digages_upload_screenshot_and_update_order_skip() {
    // Check if the required POST parameters are set
    if (isset($_POST['order_id']) && isset($_POST['payment_method_title'])) {    
